...
public void authorise_charge_notice_click_continue() throws IOException, InterruptedException
{
	...
	WebDriverWait wait = new WebDriverWait(driver_Authorise_charge_notice_popbitch, 10);
	WebElement frame = wait.until(ExpectedConditions.visibilityOfElementLocated(By.id("iframe__inpage_notices")));
	JavascriptExecutor js = (JavascriptExecutor)driver_Authorise_charge_notice_popbitch;
	js.executeScript ("document.getElementById(frame).focus()");
	Thread.sleep(6000);
